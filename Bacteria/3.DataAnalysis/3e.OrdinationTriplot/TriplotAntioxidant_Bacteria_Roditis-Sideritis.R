MUST_Bacteria_Roditis_2020.2021
MUST_Bacteria_Sideritis_2020.2021
...
AntioxidantRoditis100 <- read.table("AntioxidantRoditis100.txt", header=T,sep = "\t",row.names = 1)

fitRoditis <- envfit(ord.nmds.bray1000, AntioxidantRoditis100[,])
...
cairo_pdf("Bacteria Roditis Antioxidant.pdf", height = 7, width = 7)
...
par(adj = 1)
title(sub = "Bacteria Roditis Antioxidant")
par(adj = .5)
...
plot_ordination(MUST_Bacteria_Sideritis_2020.2021_Merged_100, ord.nmds.bray1000, color="terroir", shape ="year", label = "mynms", title=paste("Sideritis 2020/2021 (stress ",round(ord.nmds.bray1000$stress, 2),")", sep = "")) + geom_point(size = 3) + scale_color_manual(values=c("#984EA3","#FF7F00"))
...
AntioxidantSideritis100 <- read.table("AntioxidantSideritis100.txt", header=T,sep = "\t",row.names = 1)

fitSideritis <- envfit(ord.nmds.bray1000, AntioxidantSideritis100[,])
...
# start the graphics device
cairo_pdf("Bacteria Sideritis Antioxidant.pdf", height = 7, width = 7)
...
par(adj = 1)
title(sub = "Bacteria Sideritis Antioxidant")
par(adj = .5)
# add the env parameter arrows (in the rgb colouring mode we have the red, green and blue values from 0 to 255 and also the alpha value or transparency which also ranges between 0 and 255)
