...
adiv <- plot_richness(fungi_vinification_Annotated, x="stage", measures=c("InvSimpson"), color="stage", shape ="vinification")

## calculate Good's coverage estimate as well
install.packages("entropart")
library(entropart)

good <- MetaCommunity(t(fungi_vinification_Annotated@otu_table))$SampleCoverage.communities
...
alpha_wide_fact <- merge(alpha_wide, data.frame(sample_data(fungi_vinification_Annotated)), by = "row.names")
